...
describe('formatAgentMessages', () => {
  ...
  it('should handle tool calls and create ToolMessages', () => {
    ...
    expect((result[1] as ToolMessage).tool_call_id).toBe('123');
  });
  ...
  it('should handle tool calls with non-JSON args', () => {
    ...
    const result = formatAgentMessages(payload);
    expect(result).toHaveLength(2);
    expect((result[0] as AIMessage).tool_calls?.[0].args).toStrictEqual({ input: 'non-json-string' });
  });
  ...
  it('should handle complex tool calls with multiple steps', () => {
    ...
    expect((result[0] as AIMessage).tool_calls).toHaveLength(1);
    expect((result[0] as AIMessage).tool_calls?.[0]).toEqual({
      id: 'search_1',
      name: 'search',
      args: { query: 'weather in New York' },
    });
    ...
    expect((result[1] as ToolMessage).tool_call_id).toBe('search_1');
    ...
    expect((result[2] as AIMessage).tool_calls).toHaveLength(1);
    expect((result[2] as AIMessage).tool_calls?.[0]).toEqual({
      id: 'convert_1',
      name: 'convert_temperature',
      args: { temperature: 75, from: 'F', to: 'C' },
    });
    ...
    expect((result[3] as ToolMessage).tool_call_id).toBe('convert_1');
  ...
  it('should exclude ERROR type content parts', () => {
    ...
    const hasErrorContent = Array.isArray(result[0].content) && result[0].content.some(
      (item) =>
